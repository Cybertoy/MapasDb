...
public class LugaresProvider extends ContentProvider{
	...
	public static final String MIME_TYPE_ALL_ITEMS="vnd.android.cursor.dir/vnd." + AUTHORITY;
	public static final String MIME_TYPE_SINGLE_ITEM="vnd.android.cursor.item/vnd." + AUTHORITY;
	...
	public Uri insert(Uri uri, ContentValues values) {
		long rowID=getOrOpenDatabase().insert(DatabaseHandler.TABLA_DB, DatabaseHandler.NOMBRE_DB , values);
		Uri nuevaUri = null;
		switch(URI_MATCHER.match(uri)) {
		case 1:
			if(rowID > 0)
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		switch (URI_MATCHER.match(uri)) {
		case 1:
			rowsAffected = getOrOpenDatabase().delete(DatabaseHandler.TABLA_DB, selection, selectionArgs);
			break;
		case 2:
			String singleRecordId = uri.getPathSegments().get(1);
			rowsAffected = getOrOpenDatabase().delete(DatabaseHandler.TABLA_DB, DatabaseHandler.COLUMNA_ID + EQUALS + singleRecordId, selectionArgs);
			break;
		}
	...
	public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
		...
		switch (URI_MATCHER.match(uri)) {
			case 1:
				rowsAffected = getOrOpenDatabase().update(DatabaseHandler.TABLA_DB, values, selection, selectionArgs);
				break;
			case 2:
				recordId = uri.getPathSegments().get(1);
				rowsAffected = getOrOpenDatabase().update(DatabaseHandler.TABLA_DB, values, DatabaseHandler.COLUMNA_ID + EQUALS + recordId, selectionArgs);
				break;
			default:
				throw new IllegalArgumentException(	INVALID_URI_MESSAGE	+ uri);
		}
